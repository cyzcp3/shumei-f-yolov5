...
from utils.metrics import fitness, loss_fitness # cyz
...
def train(hyp,  # path/to/hyp.yaml or hyp dictionary
    ...
    start_epoch, best_fitness, best_loss = 0, 0.0, 1000.0 # cyz
    if pretrained:
        # Optimizer
        if ckpt['optimizer'] is not None:
            ...
            # cyz
            try:
                best_fitness = ckpt['best_fitness'][0]
                best_loss = ckpt['best_loss'][1]
            except:
                best_fitness = ckpt['best_fitness']
            # cyz
    ...
    # distill anchoer set
    if opt.distill and not opt.noautoanchor:
        det = model.model[-1]
        # det = model.module.model[-1] if Line.is_parallel(model) else model.model[-1]
        s_anchors = det.anchors  # shape = (3, 3, 2)
        t_det = t_model.model[-1]
        # t_det = t_model.module.model[-1] if Line.is_parallel(t_model) else t_model.model[-1]
    ...
    dist_loss = opt.dist_loss # distill loss
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        mdloss = torch.zeros(1, device=device) # mean d-losses
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            with amp.autocast(enabled=cuda):
                ...
                # distill t-model pred
                if opt.distill:
                    with torch.no_grad():
                        t_pred = t_model(imgs)

                loss, loss_items = compute_loss(pred, targets.to(device))  # loss scaled by batch_size
                # distill compute loss
                if opt.distill:
            ...
            if RANK in [-1, 0]:
                ...
                mdloss = (mdloss * i + dloss) / (i + 1) # update distill mean losses
                mem = f'{torch.cuda.memory_reserved() / 1E9 if torch.cuda.is_available() else 0:.3g}G'  # (GB)
                # pbar.set_description(('%10s' * 2 + '%10.4g' * 5) % (
                #     f'{epoch}/{epochs - 1}', mem, *mloss, targets.shape[0], imgs.shape[-1]))
                # loss print (with distill)
        ...
        if RANK in [-1, 0]:
            ...
            if fi > best_fitness:
                best_fitness = fi
            # cyz
            lfi = loss_fitness(np.array(results).reshape(1, -1)) # weighted combination of [box_loss, obj_loss, cls_loss]
            if lfi < best_loss:
                best_loss = lfi
            # cyz                    
            ...
            if (not nosave) or (final_epoch and not evolve):  # if save
                ckpt = {'epoch': epoch,
                        ...
                        'best_fitness': [best_fitness, lfi],    # cyz
                        'best_loss': [fi, best_loss],   # cyz
